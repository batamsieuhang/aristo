 ...
 */
public class TopologySubmitterCS {

    private String topologyName;
    private File file;


    public TopologySubmitterCS(String topologyName){
    ...
    public void writeData(int workers, int spout, int distribute, int general, int general2)throws IOException {
        PrintWriter writer = new PrintWriter(new FileWriter(file, true));
        String comma = ",";
        writer.println(workers + comma + spout + comma + distribute + comma + general + comma + general2);
        writer.close();
    }

    public void submitTopology(int workers, int spout, int distribute, int general, int general2) throws Exception{
        writeData(workers, spout, distribute, general, general2);
        TopologyBuilder builder = new TopologyBuilder();

        builder.setSpout("spout", new MultipleStreamSpout(1), spout);

        builder.setBolt("distribute", new MultipleStreamNumberBolt(2), distribute).shuffleGrouping("spout", "0");
        builder.setBolt("general", new GeneralNumberbolt(), general).shuffleGrouping("distribute", "0");
        builder.setBolt("general2", new GeneralNumberbolt(), general2).shuffleGrouping("distribute", "1");
...
//        conf.put(Config.TOPOLOGY_MAX_SPOUT_PENDING, 60000);
    ...
    private void monitorTopology() throws Exception{
        ...
        ComponentMetricsCreator bolt = new ComponentMetricsCreator(topologyName, "distribute");
...
//        int counter = 1;
    ...
    public static void main(String args[]) {
        try {
            ...
            TopologySubmitterCS t = new TopologySubmitterCS("testCS");

            for (int i = 1; i <= 4; i++) { //worker
                for (int j = 1; j <= i; j++) { //spout
                    for (int k = 1; k <= i; k++) { //distribute
                        for (int w = 1; w <= i; w++) { //general
                            for (int l = 1; l <= i; l++) { //general2
