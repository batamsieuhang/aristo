...
/**
 * Created by vgol on 30/1/2017.
 */
public class MoneyTimeEvaluator extends AbstractProblem {
    ...
    public MoneyTimeEvaluator(GraphParser graph, UserParameters up, SystemParameters sp) throws Exception{
        super(graph.getSpoutMap().size() + graph.getBoltMap().size() + 1, 2, 2);
    ...
    public void evaluate(Solution solution) {
        ...
        try {
            ...
            solution.setObjective(1, money);
            solution.setConstraint(0, time <= up.getDeadline() ? 0.0 : 1.0);
            solution.setConstraint(1, money <= up.getBudget() ? 0.0 : 1.0);
        }
        catch(Exception e) {
