...
public class FlowCheck {
    ...
    public int compareSpoutStats(SpoutMetrics previous, SpoutMetrics current) {
        int severity = 0;
        ...
        avgLatency += current.getCompleteLatency();
        count++;
    ...
    public void initBoltCheck(ComponentNode root) {
        ...
        if(currentConfig.getComponents().isEmpty()) {
            ...
            double totalCapacity = boltStats.getCapacity();
            while(!queue.isEmpty()) {
                bolt = queue.poll();
                boltStats = ((BoltMetricsUpdater) bolt.getNode().getComponentUpdater()).getBoltMetrics();
